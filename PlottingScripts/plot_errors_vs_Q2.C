...
void plot_errors_vs_Q2() {
  ...
  std::ifstream infile("../FinalGrids/NNPDF31_nnlo_as_0118_proton_StandardBinning_ele_Ee18_Ep275_lumi10.0fb-1_all_columns_grid.csv"); 
  std::string line;
  ...
  for (size_t i = 0; i < x_values.size(); ++i) {
    ...
    Text_com.DrawLatexNDC(.5,.85,"e+p, #sqrt{s} = 140 GeV, L_{proj} = 10 fb^{-1}");
    ...
    c1->Update();
    c1->Print(Form("error_plots/Ee18_Ep275/Error_vs_Q2_x%.1e.png",x_values[i]));
  }
