...
void make_18x275_canvas(std::vector<int> colors) {
  gStyle->SetPadLeftMargin(0.15);
  // gStyle->SetPalette(kBird);
  gROOT->ForceStyle();

  std::ifstream infile("../FinalGrids/NNPDF31_nnlo_as_0118_proton_StandardBinning_ele_Ee18_Ep275_lumi10.0fb-1_pessimistic_grid.csv"); 
  std::string line;
  ...
  legend->AddEntry(marker1, "e+p, #sqrt{s} = 28 GeV", "p");
  legend->AddEntry(marker2, "e+p, #sqrt{s} = 63 GeV", "p");
  legend->AddEntry(marker3, "e+p, #sqrt{s} = 140 GeV", "p");
...
void make_10x100_canvas(std::vector<int> colors) {
  gStyle->SetPadLeftMargin(0.15);
  // gStyle->SetPalette(kBird);
  gROOT->ForceStyle();

  std::ifstream infile("../FinalGrids/NNPDF31_nnlo_as_0118_proton_StandardBinning_ele_Ee10_Ep100_lumi10.0fb-1_pessimistic_grid.csv"); 
  std::string line;
...
void make_5x41_canvas(std::vector<int> colors) {
  gStyle->SetPadLeftMargin(0.15);
  // gStyle->SetPalette(kBird);
  gROOT->ForceStyle();

  std::ifstream infile("../FinalGrids/NNPDF31_nnlo_as_0118_proton_StandardBinning_ele_Ee5_Ep41_lumi10.0fb-1_pessimistic_grid.csv"); 
  std::string line;
...
void plot_all_com_pdf_comparison_sigma_red_scaled_vs_Q2() {
  ...
  c1->SetLogx();
  c1->SetLogy();
  make_5x41_canvas(colors);
  c1->Update();
  make_10x100_canvas(colors);
  c1->Update();
  make_18x275_canvas(colors);
  c1->Update();
  ...
  Text_com.DrawLatexNDC(.15,.85,"L_{proj} = 10 fb^{-1} per beam setting");
  TLatex Text_date;
