...
void New_Podio_ExtractYield_StandardBinning_18x275() {
  // Settings
  Float_t E_ebeam = 18;
  Float_t E_pbeam = 275;
  Float_t target_lumi = 10.;
  ...
  // "./Campaign_25.10.0/18x275/shorter_18x275_Q2_1_v25.10.0_filelist.txt",
  // "./Campaign_25.10.0/18x275/shorter_18x275_Q2_10_v25.10.0_filelist.txt",
  // "./Campaign_25.10.0/18x275/shorter_18x275_Q2_100_v25.10.0_filelist.txt",
  // "./Campaign_25.10.0/18x275/shorter_18x275_Q2_1000_v25.10.0_filelist.txt"
  // };
  std::vector<TString> filenames = {
    "./Campaign_25.10.0/18x275/merged_18x275_Q2_1.root",
    "./Campaign_25.10.0/18x275/merged_18x275_Q2_10.root",
    "./Campaign_25.10.0/18x275/merged_18x275_Q2_100.root",
    "./Campaign_25.10.0/18x275/merged_18x275_Q2_1000.root"
  };
  
  std::vector<double> gen_xsecs = {7.43E-01,6.93E-02,3.37E-03,7.94E-05}; // ub
  std::vector<double> q2_vals = {1, 10, 100, 1000};
  
  // Define logarithmically spaced bins for x and Q2
  const int nBinsX = 21;
  ...
  // Init ROOT output tree
  treewriter_init(Form("TreeOutput_StandardBinning_%gx%g.root",E_ebeam, E_pbeam));
  ...
  for (auto filename : filenames){
    ...
    auto inFiles = filenames[file_index].Data();
    
    auto reader = podio::ROOTReader();
    // reader.openFiles(inFiles);
    reader.openFile(inFiles);
    ...
    for (size_t i = 0; i < reader.getEntries("events"); i++) {// begin event loop
      ...
      if (file_index < q2_vals.size()-1){// avoid double counting
	if (Q2_truth > q2_vals[file_index+1]) continue; 
      }
    ...
    treewriter_fill(
		    x_truth,x_ele,x_jb,x_da,x_sig,x_esig,
		    y_truth,y_ele,y_jb,y_da,y_sig,y_esig,
		    Q2_truth,Q2_ele,Q2_jb,Q2_da,Q2_sig,Q2_esig,
		    E,theta,sigma_h,pt_had,sigma_h_true,pt_had_true,
		    weight);
  ...
  TFile *histFile = new TFile(Form("yield_histograms_podio_StandardBinning_%gx%g.root",E_ebeam,E_pbeam),"recreate");
  ...
  write_benchmarks_pdf(Form("benchmarks_StandardBinning_%gx%g.pdf",E_ebeam, E_pbeam), /*logx=*/false);
